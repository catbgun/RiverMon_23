...
five$Station.name <- revalue(five$Station.name, c("Målselva"="Målselva"))
...
Perhaps a good idea to have the detection frequency?
...
```{r}
Dxa2023yy <- cbind(a2023[2:3], apply(a2023[,-c(1:3), drop=F],2, dLimity))
Dxfiveyy <-  cbind(five[3:4], apply(five[,-c(1:4), drop=F],2, dLimity))

write.csv(Dxa2023yy, "C:/Users/CBG/OneDrive - NIVA/1 Projects/2022_Elveovervaakningen/RiverMonitoring_23/240624_CheckingLOformula.csv", row.names=FALSE)
```
...
names(Dxa2023yy)[names(Dxa2023yy) == 'TSM'] <- 'SPM'
```
...
Dxfiveyy$Dates <- as.Date(Dxfiveyy$Date,format = "%d.%m.%Y")
Dxfiveyy$Year <- format(Dxfiveyy$Dates, format="%Y")
names(Dxfiveyy)[names(Dxfiveyy) == 'TURB860'] <- 'Turbiditet'
...
```{r Need to handle the additional samples }
#First make date without time to be able to aggregate the samples from the same date
Dxa2023yy$Sample.dateS <- as.Date(Dxa2023yy$Sample.date,format = "%d.%m.%Y")
...
#particulate P is needed for later. must be renamed
names(Dxa2023yy)[names(Dxa2023yy) == "TOTP_P"] <- "TOTPP"

dx23_C <- Dxa2023yy %>%
  group_by(Station.name, Sample.dateS) %>%
  select(Ca:DOC, NH4.N:POC, SiO2, TOC:TOTPP, SPM:Turbiditet, As:Cd, Cr:Hg, Ni, Pb, Zn) %>% # select variables to summarise, from and to
  summarise(across(everything(), .f = list(mean = mean), na.rm = TRUE))

for ( col in 1:ncol(dx23_C)){
    colnames(dx23_C)[col] <-  sub("_.*", "", colnames(dx23_C)[col])
}

write.csv(dx23_C, "240624_CheckingDATA23_flood.csv", row.names=FALSE)
...
df.sum23$year <- "2023 mean"

names(df.sum23) <- c("Station.name", "chem", "xmean", "xsd", "year")
df.sum23 = df.sum23 %>% relocate(year, .after = "Station.name")

#Sjekke data i excel
write.csv(df.sum23, "240624_CheckingDATA23.csv", row.names=FALSE)
```
...
dfsum2 = All %>% filter(str_detect(chem,"Ca|Turbiditet|SPM|SiO2|TOC|TOTN|TOTP|As|Pb|Cd|Cu|Zn|Cr|PO4|pH|Ni"))
```
...
ggsave(filename = paste0("output8/",
                         chems[i],
                         "_plot.png"),
       plot = ploy,
...
ggsave(filename = "output8/pH22_scale.png",
       plot = pH22,
       width = 11, height = 8.5, units = "in")
...
```{r}
ggsave(filename = "output8/Si22_2.png",
       plot = Si22_2,
       width = 11, height = 8.5, units = "in")
```
...
ggsave(filename = "output8/TOTP22_Facet.png",
       plot = gp,
       width = 11, height = 8.5, units = "in")
...
ggsave(filename = "output8/PO422_Facet.png",
       plot = gp,
       width = 11, height = 8.5, units = "in")
...
ggsave(filename = "output8/Ni22_Facet.png",
       plot = gp,
       width = 11, height = 8.5, units = "in")
...
```{r}
#Dxa2023yy$POC = Dxa2023yy$TOC - Dxa2023yy$DOC
#Dxa2023yy$TDP = Dxa2023yy$TOTP - Dxa2023yy$TOTPP #NOT SURE IF THIS IS CORRECT
#Dxa2023yy$Org.P = Dxa2023yy$TOTP - Dxa2023yy$PO4.P

#Dxa2023yy$Org.N = Dxa2023yy$TOTN - Dxa2023yy$NO3.N - Dxa2023yy$NH4.N
#Dxa2023yy$TDN = Dxa2023yy$TOTN - Dxa2023yy$Nitrogen.part
```
...
Hgdfsum3 <- AllHgx %>% 
  group_by(year) %>% 
           mutate(year=factor(year,
                     levels=c("3-year mean", "2023 mean")))
